...
export const authApi = {
  signup: async (name: string, email: string, password: string, defaultCurrency = 'INR') => {
    const response = await api.post('/auth/signup', { name, email, password, defaultCurrency })
...
export const expensesApi = {
  ...
  createPersonal: async (title: string, amount: number, currency: string, date?: string) => {
    const response = await api.post('/expenses/personal', { title, amount, currency, date })
    return response.data
  }
}
